  ...
  componentDidUpdate (prevProps) {
    if (prevProps.options !== this.props.options) {
      ...
      this.setState((prevState) => {
        // Keep current selection when still valid; otherwise fallback to first option.
        const hasCurrent = options.some(item => item && item.value === prevState.selectValue);
        return {
          selectValue: hasCurrent ? prevState.selectValue : (options[0] ? options[0].value : '')
        };
      });
    }
  }
